...
def get_fairness_metrics(atts_n_vals,path_to_csv):
    ...
    score_data,label_data=preprocess_dataset(df,catCols,numCols,target_cols)
    ...
    for element in atts_n_vals:
        
        truth = BinaryLabelDataset(df=label_data,
                                        label_names=['Score'],
                                        protected_attribute_names=[element['value']],
                                        favorable_label=1,
                                        unfavorable_label=0,
                                        )
        ...
        metric=ClassificationMetric(truth, 
                                        preds, 
                                        unprivileged_groups=[{element['value']: 0 }], 
                                        privileged_groups=[{element['value']: 1}],
                                        )

        metric2=BinaryLabelDatasetMetric(truth, 
                                        unprivileged_groups=[{element['value']: 0 }], 
                                        privileged_groups=[{element['value']: 1}],
                                        )
        ...
        tn, fp, fn, tp = confusion_matrix(score_data['Score'], label_data['Score']).ravel()
        details=[{'Accuracy': metrics.accuracy_score(score_data['Score'], label_data['Score']), 'Precision':metrics.precision_score(score_data['Score'], label_data['Score']),'Recall': metrics.recall_score(score_data['Score'], label_data['Score']),'True negatives': tn,'False positives': fp,'False negatives': fn,'True positives': tp}]
    return list_of_metrics,list_of_b_metrics,list_of_datasets,label_data,score_data,details
            ...
            for att in element['Protected_Attributes']:
                ...
                if (method=="Reweighing"):
                    ...
                    if (element['Metric']=='mean_difference' or element['Metric']=='disparate_impact'):
                        blm=BinaryLabelDatasetMetric(rw_label_data,unprivileged_groups=unprivileged_groups,privileged_groups=privileged_groups)
                    ...
                    if flag1:
                        ...
                        temp2={'Accuracy': metrics.accuracy_score(dataset.labels, data_all.labels), 'Precision':metrics.precision_score(dataset.labels, data_all.labels),'Recall': metrics.recall_score(dataset.labels, data_all.labels),'True negatives': tn,'False positives': fp,'False negatives': fn,'True positives': tp,'Method':method}
                    ...
                    if (element['Metric']=='mean_difference' or element['Metric']=='disparate_impact'):
                        blm=BinaryLabelDatasetMetric(data_all,unprivileged_groups=unprivileged_groups,privileged_groups=privileged_groups)
                        value=getattr(blm, element['Metric'])()
                    else:
                        preds=ClassificationMetric(dataset,data_all,  
                                            unprivileged_groups=[{att: 0 }], 
                    ...
                    if (element['Metric']=='mean_difference' or element['Metric']=='disparate_impact'):
                        blm=BinaryLabelDatasetMetric(debiased,unprivileged_groups=unprivileged_groups,privileged_groups=privileged_groups)
                        value=getattr(blm, element['Metric'])()
                    else:
                    ...
                    if flag3:
                        tn, fp, fn, tp = confusion_matrix(bias.labels,debiased.labels).ravel()
                        temp2={'Accuracy': metrics.accuracy_score(bias.labels,debiased.labels), 'Precision':metrics.precision_score(bias.labels,debiased.labels),'Recall': metrics.recall_score(bias.labels,debiased.labels),'True negatives': tn,'False positives': fp,'False negatives': fn,'True positives': tp,'Method':method}
                        model_info.append(temp2)
                        flag3=False
                    ...
                    if flag5:
                        ...
                        temp2={'Accuracy': metrics.accuracy_score(y_test_pred.labels,dataset2.labels), 'Precision':metrics.precision_score(y_test_pred.labels,dataset2.labels),'Recall': metrics.recall_score(y_test_pred.labels,dataset2.labels),'True negatives': tn,'False positives': fp,'False negatives': fn,'True positives': tp,'Method':method}
                        model_info.append(temp2)
                        flag5=False           
